...
unsigned char baudot_shift_rcv = LTRS;
unsigned char baudot_shift_send = LTRS;
...
const char ltrs[32] = { 0, 'E', 0x0A, 'A', ' ', 'S', 'I', 'U', 
		0x0D, 'D', 'R', 'J', 'N', 'F', 'C', 'K', 
		'T', 'Z', 'L', 'W', 'H', 'Y', 'P', 'Q', 
		'O', 'B', 'G', 0, 'M', 'X', 'V', 0 };
const char figs[32] = { 0, '3', 0x0A, '-', ' ', 7, '8', '7', 
		0x0D, '$', '4', 0x27, ',', '!', ':', '(', 
		'5', '"', ')', '2', '#', '6', '0', '1', 
		'9', '?', '&', 0, '.', '/', ';', 0 };
...
char baudot_to_ascii(char b) { 
    ...
    if (baudot_shift_rcv == LTRS) 
      asc = ltrs[(unsigned char)b];
    else if (baudot_shift_rcv == FIGS)
      asc = figs[(unsigned char)b];
